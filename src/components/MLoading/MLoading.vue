...
const normalizedValue = computed(() => Math.max(0, Math.min(100, parseFloat(String(props.value)))))
const diameter = computed(() => (RADIUS / (1 - width.value / size.value)) * 2)
const strokeWidth = computed(() => (width.value / size.value) * diameter.value)
const strokeDashOffset = computed(() => ((100 - normalizedValue.value) / 100) * CIRCUMFERENCE)
</script>

<template>
  <div class="m-loading" :style="loadingStyle" :class="loadingClass">
    <svg :style="svgStyle" xmlns="http://www.w3.org/2000/svg" :viewBox="viewBox">
      <circle
